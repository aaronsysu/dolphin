...
::rtspURL(ServerMediaSession const* serverMediaSession, int clientSocket) const {
  ...
  sprintf_s(resultURL, strlen(urlPrefix) + strlen(sessionName) + 1, "%s%s", urlPrefix, sessionName);
...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
  ...
  if (portNumHostOrder == 554 /* the default port number */) {
    sprintf_s(urlBuffer, "rtsp://%s/", AddressString(ourAddress).val());
  } else {
    sprintf_s(urlBuffer, "rtsp://%s:%hu/",
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
	...
	sprintf_s(concatenatedStreamName, strlen(urlPreSuffix) + strlen(urlSuffix) + 2, "%s/%s", urlPreSuffix, urlSuffix);
    ...
    if (fOurRTSPServer.fReclamationSeconds > 0) {
      sprintf_s(timeoutParameterString, ";timeout=%u", fOurRTSPServer.fReclamationSeconds);
    } else {
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  if (absStart != NULL) {
    ...
    if (absEnd == NULL) {
      sprintf_s(buf, "Range: clock=%s-\r\n", absStart);
    } else {
      sprintf_s(buf, "Range: clock=%s-%s\r\n", absStart, absEnd);
    }
    delete[] absStart; delete[] absEnd;
  } else {
    ...
    if (rangeEnd == 0.0 && scale >= 0.0) {
      sprintf_s(buf, "Range: npt=%.3f-\r\n", rangeStart);
    } else {
      sprintf_s(buf, "Range: npt=%.3f-%.3f\r\n", rangeStart, rangeEnd);
    }
  }
  ...
  for (i = 0; i < fNumStreamStates; ++i) {
	...
	+ 2 /*allows for trailing \r\n at final end of string*/;
	  ...
	  sprintf_s(rtpInfo, rtpInfoSize, rtpInfoFmt,
	      prevRTPInfo,
	      numRTPInfoItems++ == 0 ? "" : ",",
	      rtspURL, urlSuffix,
	      rtpSeqNum,
...
char const* RTSPServerWithREGISTERProxying::allowedCommandNames() {
  if (fAllowedCommandNames == NULL) {
	...
	sprintf_s(fAllowedCommandNames, strlen(baseAllowedCommandNames) + strlen(newAllowedCommandName) + 1, "%s%s", baseAllowedCommandNames, newAllowedCommandName);
  }
...
void RTSPServerWithREGISTERProxying::implementCmd_REGISTER(char const* url, char const* /*urlSuffix*/, int socketToRemoteServer,
  ...
  if (proxyURLSuffix == NULL) {
    sprintf_s(proxyStreamNameBuf, "registeredProxyStream-%u", ++fRegisteredProxyCounter);
