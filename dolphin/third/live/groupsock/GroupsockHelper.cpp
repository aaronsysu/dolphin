...
int setupDatagramSocket(UsageEnvironment& env, Port port) {
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
    ...
    if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      char tmpBuffer[100];
	  sprintf_s(tmpBuffer, "bind() error (port number: %d): ",
	      ntohs(port.num()));
...
Boolean writeSocket(UsageEnvironment& env,
  ...
  do {
    ...
    if (bytesSent != (int)bufferSize) {
      char tmpBuf[100];
	  sprintf_s(tmpBuf, "writeSocket(%d), sendTo() error: wrote %d bytes instead of %u: ", socket, bytesSent, bufferSize);
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  if (ourAddress == 0) {
    ...
    if (badAddressForUs(from)) {
      char tmp[100];
      sprintf_s(tmp, "This computer has an invalid IP address: %s", AddressString(from).val());
...
char const* timestampString() {
  ...
  char* ctimeResult = NULL;
  size_t ctimeSize = 0;
  ctime_s(ctimeResult, ctimeSize, &tvNow_t);
  if (ctimeResult == NULL) {
