...
char const* Authenticator::computeDigestResponse(char const* cmd,
  ...
  if (fPasswordIsMD5) {
	...
	sprintf_s((char*)ha1Data, ha1DataLen + 1, "%s:%s:%s", username(), realm(), password());
  ...
  sprintf_s((char*)ha2Data, ha2DataLen + 1, "%s:%s", cmd, url);
  char ha2Buf[33];
  ...
  sprintf_s((char*)digestData, digestDataLen + 1, "%s:%s:%s",
          ha1Buf, nonce(), ha2Buf);
