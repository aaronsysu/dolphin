...
static Boolean setGroupsockBySocket(UsageEnvironment& env, int sock,
  ...
  do {
    ...
    if (sock < 0) {
      char buf[100];
      sprintf_s(buf, "trying to use bad socket (%d)", sock);
    ...
    if (alreadyExists) {
      char buf[100];
	  sprintf_s(buf, "Attempting to replace an existing socket (%d)", sock);
      env.setResultMsg(buf);
      break;
    }
