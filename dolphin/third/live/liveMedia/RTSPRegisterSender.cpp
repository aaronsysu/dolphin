...
::RTSPRegisterSender(UsageEnvironment& env,
  ...
  sprintf_s(fakeRTSPURL, fakeRTSPURLSize, fakeRTSPURLFmt, remoteClientNameOrAddress, remoteClientPortNum);
...
Boolean RTSPRegisterSender::setRequestFields(RequestRecord* request,
  ...
  if (strcmp(request->commandName(), "REGISTER") == 0) {
	...
	+ strlen(request_REGISTER->proxyURLSuffix());
	  ...
	  sprintf_s(proxyURLSuffixParameterStr, proxyURLSuffixParameterSize, proxyURLSuffixParameterFmt, request_REGISTER->proxyURLSuffix());
    }
	...
	sprintf_s(transportHeaderStr, transportHeaderSize, transportHeaderFmt,
