...
unsigned RTSPClient::sendSetParameterCommand(MediaSession& session, responseHandler* responseHandler,
  ...
  sprintf_s(paramString, strlen(parameterName) + strlen(parameterValue) + 10, "%s: %s\r\n", parameterName, parameterValue);
...
unsigned RTSPClient::sendGetParameterCommand(MediaSession& session, responseHandler* responseHandler, char const* parameterName,
  ...
  if (parameterName == NULL) {
    paramString[0] = '\0';
	  ...
	  sprintf_s(paramString, parameterNameLen + 3, "%s\r\n", parameterName);
  }
...
void RTSPClient::setUserAgentString(char const* userAgentName) {
  ...
  sprintf_s(fUserAgentHeaderStr, headerSize, formatStr, userAgentName);
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  sprintf_s(userAgentName, userAgentNameSize, "%s%s%s%s%s", applicationName, libPrefix, libName, libVersionStr, libSuffix);
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
	...
	if (numExtraBytesNeeded > remainingBufferSize) {
	  char tmpBuf[200];
	  sprintf_s(tmpBuf, "Response buffer size (%d) is too small for \"Content-Length:\" %d (need a buffer size of >= %d bytes\n",
