...
::handleHTTPCmd_StreamingGET(char const* urlSuffix, char const* /*fullRequestStr*/) {
  ...
  char* s = playlist;
  sprintf_s(s, playlistMaxSize, playlistPrefixFmt, targetDuration);
  s += strlen(s);
  ...
  while (1) {
    ...
    duration -= dur;
	sprintf_s(s, playlistMaxSize, playlistMediaFileSpecFmt, dur, urlSuffix, durSoFar, dur);
    s += strlen(s);
  ...
  sprintf_s(s, playlistMaxSize, playlistSuffixFmt);
  s += strlen(s);
