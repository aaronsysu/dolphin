...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
  ...
  if (info == NULL || description == NULL) {
	...
	sprintf_s(libNamePlusVersionStr, strlen(libNameStr) + strlen(libVersionStr) + 1, "%s%s", libNameStr, libVersionStr);
  }
...
char* ServerMediaSession::generateSDPDescription() {
  ...
  if (fIsSSM) {
	...
	sprintf_s(sourceFilterLine, sourceFilterFmtSize, sourceFilterFmt, ipAddressStr.val());
  } else {
  ...
  do {
    ...
    if (dur == 0.0) {
      ...
      char buf[100];
      sprintf_s(buf, "a=range:npt=0-%.3f\r\n", dur);
...
char const* ServerMediaSubsession::trackId() {
  ...
  if (fTrackId == NULL) {
    char buf[100];
    sprintf_s(buf, "track%d", fTrackNumber);
    fTrackId = strDup(buf);
  }
  return fTrackId;
}
...
ServerMediaSubsession::rangeSDPLine() const {
  ...
  if (absStart != NULL) {
    ...
    if (absEnd != NULL) {
      sprintf_s(buf, "a=range:clock=%s-%s\r\n", absStart, absEnd);
    } else {
      sprintf_s(buf, "a=range:clock=%s-\r\n", absStart);
    }
    return strDup(buf);
  }
